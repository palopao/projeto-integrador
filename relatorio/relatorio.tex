...
\midrule
1 --- Análise e Dados &  & Recolha dos dados DGES (2017--2025), estruturação dos ficheiros JSON, análise exploratória, definição de requisitos. \\
2 --- Motor de Simulação &  & Implementação do gerador de populações e do simulador Gale-Shapley; primeiros testes de validação. \\
3 --- Modelo de IA &  & Implementação do módulo de treino com \textit{backpropagation} e \textit{momentum}; calibração de hiperparâmetros; treino 2017--2024. \\
4 --- \textit{Frontend} &  & Arquitetura React; implementação dos componentes; integração dos dados; visualizações com Recharts. \\
5 --- Previsão e IC &  & Algoritmo de suavização exponencial com reversão à média; cálculo de intervalos de confiança de 95\%. \\
6 --- Validação &  & \textit{Backtesting} do algoritmo de previsão; análise de erros; ajuste de parâmetros; melhorias de usabilidade. \\
7 --- Documentação &  & Elaboração do relatório final. \\
\bottomrule
\end{tabularx}
\end{table}
...
Este módulo replica computacionalmente o processo de colocação da DGES, que segue uma variante do algoritmo Gale-Shapley:
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Gráficos}: visualizações implementadas com \textbf{Recharts}, uma biblioteca baseada em SVG para React.
  \item \textbf{Estilos}: CSS Modules para estilos com âmbito local, evitando conflitos entre componentes.
\end{itemize}

O algoritmo de previsão adota uma abordagem diferenciada por fase, motivada pelos resultados de validação:

\begin{itemize}[noitemsep]
  \item \textbf{1.ª Fase}: a previsão principal é fornecida diretamente pelo motor de simulação Gale-Shapley (nota simulada), que, como demonstrado na Secção~\ref{sec:validacao}, atinge MAE = 5.2 pontos nesta fase e supera a suavização exponencial isolada. Os intervalos de confiança de 95\% são calculados pela suavização exponencial, garantindo uma margem estatisticamente fundamentada em torno do valor simulado.
  \item \textbf{2.ª e 3.ª Fases}: dado que o simulador apresenta erro significativamente maior nestas fases (MAE = 15.4 pontos na 2.ª Fase), a previsão é obtida exclusivamente por \textbf{suavização exponencial} com $\alpha = 0.5$ e reversão progressiva à média histórica, método mais robusto para as fases com menor volume de candidatos e maior volatilidade.
\end{itemize}

O algoritmo de suavização exponencial, utilizado diretamente nas Fases~2 e~3 e para os intervalos de confiança da Fase~1, segue:
...
\end{equation}
\begin{equation}
\hat{y}_{t+1} = (1 - w_1) \cdot S_T + w_1 \cdot \bar{y}, \quad w_1 = 0.15
\label{eq:prediction}
\end{equation}

onde $S_T$ é o valor suavizado no último ano observado e $\bar{y}$ é a média histórica. A margem do intervalo de confiança é $\text{margem} = 1.28 \cdot \hat{\sigma} \cdot 1.15$, onde $\hat{\sigma}$ é o desvio-padrão residual. A plataforma apresenta a previsão para o \textbf{ano imediatamente seguinte}, utilizando um horizonte único para maximizar a fiabilidade da estimativa.
...
\begin{itemize}
  \item \textbf{Gráfico de evolução por fase}: representação temporal das notas de acesso observadas (2017--2025) e da previsão para o ano seguinte (2026), com banda de confiança a 95\%. O cabeçalho do painel destaca de imediato o intervalo previsto para 2026 na 1.ª Fase. A previsão da 1.ª Fase usa diretamente o resultado do simulador; as Fases~2 e~3 usam suavização exponencial.
...
\subsubsection{Validação do Algoritmo de Previsão --- 1.ª Fase}

A estratégia de previsão diferenciada descrita na Secção~\ref{sec:solucao} foi validada por \textit{backtesting} na 1.ª Fase: para cada ano de 2019 a 2025, o modelo foi alimentado com dados até ao ano anterior e a previsão comparada com os valores reais. A Figura~\ref{fig:comparacao} compara diretamente a suavização exponencial isolada com o sistema de simulação completo.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{comparacao_simulador_vs_exp_fase_1.png}
  \caption{Comparação entre suavização exponencial e o sistema de simulação em \textit{backtesting} para a 1.ª Fase (2019--2025).}
  \label{fig:comparacao}
\end{figure}

O sistema de simulação obteve MDA global de \textbf{5.16~pts} face a 7.88~pts da suavização exponencial isolada, confirmando a vantagem da abordagem baseada em IA para a 1.ª Fase. O pico de erro em 2020--2021 deve-se às anomalias introduzidas pela pandemia de COVID-19, que produziram uma inflação atípica das notas de acesso não prevista pelos dados históricos anteriores.

Para as \textbf{2.ª e 3.ª Fases}, onde o simulador apresenta maior erro (MAE = 15.4 e 21.5 pontos, respetivamente), é usada exclusivamente a suavização exponencial como método de previsão, constituindo uma solução mais estável face à maior volatilidade e ao menor volume de candidatos nestas fases.
...
\begin{itemize}[noitemsep]
  ...
  \item Uma estratégia de previsão diferenciada por fase: simulador Gale-Shapley para a 1.ª Fase (MDA de \textit{backtesting} = 5.16 pontos) e suavização exponencial para as 2.ª e 3.ª Fases, com intervalos de confiança de 95\%.
...
\begin{itemize}
  ...
  \item \textbf{Equilíbrio entre complexidade e precisão (A)}: a validação mostrou que o simulador vence na 1.ª Fase (MDA = 5.16 pts vs.\ 7.88 pts da suavização exponencial), mas perde nas Fases~2 e~3 (MAE = 15.4 e 21.5 pts). Esta assimetria levou à adoção de uma estratégia híbrida: o melhor método é selecionado por fase com base em resultados empíricos, não em intuição sobre a complexidade do modelo.
...
\begin{enumerate}[noitemsep]
  ...
  \item Exploração de modelos de séries temporais mais sofisticados (ARIMA, Prophet) para comparação com a abordagem atual.
